...
void
user_handler(struct proc *curr_p, int sig_no)
{
  uint user_esp = curr_p->tf->esp; // esp of user stack
  
  // backup the trapframe
  user_esp -= sizeof(struct trapframe);
  memmove((void*)user_esp,(void*)curr_p->tf,sizeof(struct trapframe)); // push traprame on user stack
  curr_p->backup_tf = (struct trapframe *)user_esp;

  // push the code of sigret on user stack
  void *sigret_addr = (void*)start_sigret;
  uint sigret_size = ((uint)&end_sigret -(uint)start_sigret);
  user_esp -= sigret_size; // move the esp to upto the sigret code size 
  uint sigret_ret_addr = user_esp;
  memmove((void*)user_esp,sigret_addr,sigret_size);

  //push the signo on user stack
  user_esp -= sizeof(uint);
  *((uint *)user_esp) = sig_no;

 //move the return address of sigret on user stack
 user_esp -= sizeof(uint) ;
 memmove((void*)user_esp,(void*)&sigret_ret_addr,sizeof(uint));

 curr_p->tf->esp = user_esp; // modify the esp 
 curr_p->tf->eip = (uint)curr_p->sig_handler[sig_no];  //modify the current instruction pointer
 return;
}
